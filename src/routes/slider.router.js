...
const permitCreate = permit("slider", ["create"]);
router
  .route("/")
  .post(
    permitCreate,
...
const permitFind = permit("slider", ["readAll"]);
...
const permitOne = permit("slider", ["read"]);
router
  .route("/:id")
  .get(permitOne, validator.params(readSchema), Controller.findOne);

const permitUpdate = permit("slider", ["update"]);
router
  .route("/:id")
  .put(
    permitUpdate,
...
const permitDelete = permit("slider", ["delete"]);
router
  .route("/:id")
