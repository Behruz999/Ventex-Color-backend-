...
const Controller = require("../controllers/photoMedia.controller");
...
const {
  ...
  findAll,
  paginate,
} = require("../validators/photoMedia.validator");
const permit = require("../util/permission");
const { uploadFiles } = require("../util/uploadFile");

router.use(authenticate);

const permitCreate = permit("photoMedia", ["create"]);
router
  .route("/")
  .post(
    permitCreate,
...
const permitFind = permit("photoMedia", ["readAll"]);
...
const permitOne = permit("photoMedia", ["read"]);
router
  .route("/:id")
  .get(permitOne, validator.params(readSchema), Controller.findOne);

const permitUpdate = permit("photoMedia", ["update"]);
router
  .route("/:id")
  .put(
    permitUpdate,
...
const permitDelete = permit("photoMedia", ["delete"]);
router
  .route("/:id")
