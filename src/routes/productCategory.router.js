...
const permitCreate = permit("productCategory", ["create"]);
router
  .route("/")
  .post(
    permitCreate,
...
const permitFind = permit("productCategory", ["readAll"]);
...
const permitOne = permit("productCategory", ["read"]);
router
  .route("/:id")
  .get(permitOne, validator.params(readSchema), Controller.findOne);

const permitUpdate = permit("productCategory", ["update"]);
router
  .route("/:id")
  .put(
    permitUpdate,
...
const permitDelete = permit("productCategory", ["delete"]);
router
  .route("/:id")
