...
const permitCreate = permit("news", ["create"]);
router
  .route("/")
  .post(
    permitCreate,
...
const permitFind = permit("news", ["readAll"]);
...
const permitOne = permit("news", ["read"]);
router
  .route("/:id")
  .get(permitOne, validator.params(readSchema), Controller.findOne);

const permitUpdate = permit("news", ["update"]);
router
  .route("/:id")
  .put(
    permitUpdate,
...
const permitDelete = permit("news", ["delete"]);
router
  .route("/:id")
