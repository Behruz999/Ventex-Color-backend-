...
const Controller = require("../controllers/productPhoto.controller");
...
const {
  ...
  findAll,
  paginate,
} = require("../validators/productPhoto.validator");
const permit = require("../util/permission");
const { uploadFiles } = require("../util/uploadFile");

router.use(authenticate);

const permitCreate = permit("productPhoto", ["create"]);
router
  .route("/")
  .post(
    permitCreate,
...
const permitFind = permit("productPhoto", ["readAll"]);
...
const permitOne = permit("productPhoto", ["read"]);
router
  .route("/:id")
  .get(permitOne, validator.params(readSchema), Controller.findOne);

const permitUpdate = permit("productPhoto", ["update"]);
router
  .route("/:id")
  .put(
    permitUpdate,
...
const permitDelete = permit("productPhoto", ["delete"]);
router
  .route("/:id")
