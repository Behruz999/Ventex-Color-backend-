...
const permitCreate = permit("about", ["create"]);
router
  .route("/")
  .post(
    permitCreate,
...
const permitFind = permit("about", ["readAll"]);
...
const permitOne = permit("about", ["read"]);
router
  .route("/:id")
  .get(permitOne, validator.params(readSchema), Controller.findOne);

const permitUpdate = permit("about", ["update"]);
router
  .route("/:id")
  .put(
    permitUpdate,
...
const permitDelete = permit("about", ["delete"]);
router
  .route("/:id")
